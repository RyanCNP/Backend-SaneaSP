'use strict';

module.exports = {
  async up(queryInterface, Sequelize) {
    const transaction = await queryInterface.sequelize.transaction();
    try {
      await queryInterface.bulkInsert('imagem_denuncia', [
        // Denúncia 1 - 2 imagens
        {
          nome: "user1.jpg",
          id_denuncia: 1
        },
        {
          nome: "user2.jpg",
          id_denuncia: 1
        },
  
        // Denúncia 2 - 2 imagens
        {
          nome: "user2.jpg",
          id_denuncia: 2
        },
        {
          nome: "user3.jpg",
          id_denuncia: 2
        },
  
        // Denúncia 3 - 2 imagens
        {
          nome: "user3.jpg",
          id_denuncia: 3
        },
        {
          nome: "user4.jpg",
          id_denuncia: 3
        },
  
        // Denúncia 4 - 1 imagem
        {
          nome: "user4.jpg",
          id_denuncia: 4
        },
  
        // Denúncia 5 - 2 imagens
        {
          nome: "user5.jpg",
          id_denuncia: 5
        },
        {
          nome: "user1.jpg",
          id_denuncia: 5
        },
  
        // Denúncia 6 - 1 imagem
        {
          nome: "user1.jpg",
          id_denuncia: 6
        },
  
        // Denúncia 7 - 2 imagens
        {
          nome: "user2.jpg",
          id_denuncia: 7
        },
        {
          nome: "user3.jpg",
          id_denuncia: 7
        },
  
        // Denúncia 8 - 1 imagem
        {
          nome: "user3.jpg",
          id_denuncia: 8
        },
  
        // Denúncia 9 - 2 imagens
        {
          nome: "user4.jpg",
          id_denuncia: 9
        },
        {
          nome: "user5.jpg",
          id_denuncia: 9
        },
  
        // Denúncia 10 - 1 imagem
        {
          nome: "user5.jpg",
          id_denuncia: 10
        },
  
        // Denúncia 11 - 2 imagens
        {
          nome: "user1.jpg",
          id_denuncia: 11
        },
        {
          nome: "user2.jpg",
          id_denuncia: 11
        },
  
        // Denúncia 12 - 1 imagem
        {
          nome: "user2.jpg",
          id_denuncia: 12
        },
  
        // Denúncia 13 - 2 imagens
        {
          nome: "user3.jpg",
          id_denuncia: 13
        },
        {
          nome: "user4.jpg",
          id_denuncia: 13
        },
  
        // Denúncia 14 - 1 imagem
        {
          nome: "user4.jpg",
          id_denuncia: 14
        },
  
        // Denúncia 15 - 2 imagens
        {
          nome: "user5.jpg",
          id_denuncia: 15
        },
        {
          nome: "user1.jpg",
          id_denuncia: 15
        },
  
        // Denúncia 16 - 1 imagem
        {
          nome: "user1.jpg",
          id_denuncia: 16
        },
  
        // Denúncia 17 - 2 imagens
        {
          nome: "user2.jpg",
          id_denuncia: 17
        },
        {
          nome: "user3.jpg",
          id_denuncia: 17
        },
  
        // Denúncia 18 - 1 imagem
        {
          nome: "user3.jpg",
          id_denuncia: 18
        },
  
        // Denúncia 19 - 2 imagens
        {
          nome: "user4.jpg",
          id_denuncia: 19
        },
        {
          nome: "user5.jpg",
          id_denuncia: 19
        },
  
        // Denúncia 20 - 1 imagem
        {
          nome: "user5.jpg",
          id_denuncia: 20
        },
  
        // Denúncia 21 - 2 imagens
        {
          nome: "user1.jpg",
          id_denuncia: 21
        },
        {
          nome: "user2.jpg",
          id_denuncia: 21
        },
  
        // Denúncia 22 - 1 imagem
        {
          nome: "user2.jpg",
          id_denuncia: 22
        },
  
        // Denúncia 23 - 2 imagens
        {
          nome: "user3.jpg",
          id_denuncia: 23
        },
        {
          nome: "user4.jpg",
          id_denuncia: 23
        },
  
        // Denúncia 24 - 1 imagem
        {
          nome: "user4.jpg",
          id_denuncia: 24
        },
  
        // Denúncia 25 - 2 imagens
        {
          nome: "user5.jpg",
          id_denuncia: 25
        },
        {
          nome: "user1.jpg",
          id_denuncia: 25
        },
  
        // Denúncia 26 - 1 imagem
        {
          nome: "user1.jpg",
          id_denuncia: 26
        },
  
        // Denúncia 27 - 2 imagens
        {
          nome: "user2.jpg",
          id_denuncia: 27
        },
        {
          nome: "user3.jpg",
          id_denuncia: 27
        },
  
        // Denúncia 28 - 1 imagem
        {
          nome: "user3.jpg",
          id_denuncia: 28
        },
  
        // Denúncia 29 - 2 imagens
        {
          nome: "user4.jpg",
          id_denuncia: 29
        },
        {
          nome: "user5.jpg",
          id_denuncia: 29
        },
  
        // Denúncia 30 - 1 imagem
        {
          nome: "user5.jpg",
          id_denuncia: 30
        },
  
        // Denúncia 31 - 2 imagens
        {
          nome: "user1.jpg",
          id_denuncia: 31
        },
        {
          nome: "user2.jpg",
          id_denuncia: 31
        },
  
        // Denúncia 32 - 1 imagem
        {
          nome: "user2.jpg",
          id_denuncia: 32
        },
  
        // Denúncia 33 - 2 imagens
        {
          nome: "user3.jpg",
          id_denuncia: 33
        },
        {
          nome: "user4.jpg",
          id_denuncia: 33
        },
  
        // Denúncia 34 - 1 imagem
        {
          nome: "user4.jpg",
          id_denuncia: 34
        },
  
        // Denúncia 35 - 2 imagens
        {
          nome: "user5.jpg",
          id_denuncia: 35
        },
        {
          nome: "user1.jpg",
          id_denuncia: 35
        },
  
        // Denúncia 36 - 1 imagem
        {
          nome: "user1.jpg",
          id_denuncia: 36
        },
  
        // Denúncia 37 - 2 imagens
        {
          nome: "user2.jpg",
          id_denuncia: 37
        },
        {
          nome: "user3.jpg",
          id_denuncia: 37
        },
  
        // Denúncia 38 - 1 imagem
        {
          nome: "user3.jpg",
          id_denuncia: 38
        },
  
        // Denúncia 39 - 2 imagens
        {
          nome: "user4.jpg",
          id_denuncia: 39
        },
        {
          nome: "user5.jpg",
          id_denuncia: 39
        },
  
        // Denúncia 40 - 1 imagem
        {
          nome: "user5.jpg",
          id_denuncia: 40
        }
      ], {});
      await transaction.commit();
    } catch (error) {
       await transaction.rollback();
      console.log('Erro na seed INSERT-IMAGEM - rollback realizado:', error.errors || error.message);
      throw error;
    }
  },

  async down(queryInterface, Sequelize) {
    await queryInterface.bulkDelete('imagem_denuncia', null, {});
    await queryInterface.sequelize.query(
      "DELETE FROM sqlite_sequence WHERE name='imagem_denuncia';"
    );
  }
};