...
export const createCategoryDenuncia = async (
  categorias: number[],
  id_denuncia: number,
) => {
  await categoryIdExistsValidator(categorias);

  await CategoriaDenunciaModel.bulkCreate(
    categorias.map((id_categoria) => ({
      id_categoria,
      id_denuncia,
    })),
  );
...
export const updateCategoryDenuncia = async (
  updatedCategoriesIds: number[],
  id_denuncia: number,
) => {
  const oldCategories = await CategoriaDenunciaModel.findAll({
    where: { id_denuncia },
  });
  ...
  const categoriesToRemove = oldCategoryIds.filter(
    (category) => !updatedCategoriesIds.includes(category),
  );
  ...
  const categoriesToCreate = updatedCategoriesIds.filter(
    (category) => !oldCategoryIds.includes(category),
  );
...
export const categoryIdExistsValidator = async (categories: number[]) => {
  ...
  if (existingCategories.length != categories.length) {
    ...
    throw new ApiError(
      invalidCategoriesIds.length == 1
        ? `A categoria com ID: ${invalidCategoriesIds} não existe`
        : `As categorias com ID: ${invalidCategoriesIds.join(", ")} não existem`,
      HttpCode.NotFound,
    );
  }
