...
export const findComentarioById = async (
  idComentario: number,
): Promise<IComentario> => {
  const comentario: IComentario | null = await ComentarioModel.findByPk(
    idComentario,
    {
      include: [
        {
  ...
  if (!comentario) {
    throw new ApiError(
      "Não foi possível encontrar Comentário com esse ID",
      HttpCode.NotFound,
    );
  }
  return comentario;
...
export const findAllComentariosByDenuncia = async (
  id: number,
): Promise<IComentario[]> => {
  ...
  if (comentarios.length === 0) {
    throw new ApiError(
      "Não foi possivel encontrar nenhum comentario relacionado a essa denuncia",
      HttpCode.NotFound,
    );
  }
  return comentarios;
...
export const findAllComententarios = async (
  idUsuario?: number,
): Promise<IComentario[]> => {
  let where = {};
  if (idUsuario) {
